...
def run_daily_organizer(root_path: str):
    ...
    categorized_new_files = organizer.daily_organize_files()
...
def save(categorized_new_files):
    folder_structure_json_path = FileManagerHelper.construct_path(FileManagerHelper.cwd, "FolderStructuringJson")
    FileManagerHelper.create_path(folder_structure_json_path)
    file_path = FileManagerHelper.construct_path(folder_structure_json_path, f"{TimeHelper.get_current_date()}_categorized_files.json")
